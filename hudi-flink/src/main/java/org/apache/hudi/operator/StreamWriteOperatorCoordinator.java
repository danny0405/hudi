...
public class StreamWriteOperatorCoordinator
  ...
  private void initTable() throws IOException {
    final String basePath = this.conf.getString(HoodieOptions.PATH);
    ...
    try (FileSystem fs = FSUtils.getFs(basePath, hadoopConf)) {
      if (!fs.exists(new Path(basePath, HoodieTableMetaClient.METAFOLDER_NAME))) {
        HoodieTableMetaClient.initTableType(
            hadoopConf,
            basePath,
            HoodieTableType.valueOf(this.conf.getString(HoodieOptions.TABLE_TYPE)),
            this.conf.getString(HoodieOptions.TABLE_NAME),
            "archived",
            this.conf.getString(HoodieOptions.PAYLOAD_CLASS),
            1);
        LOG.info("Table initialized");
      } else {
        LOG.info("Table [{}/{}] already exists, no need to initialize the table",
            basePath, this.conf.getString(HoodieOptions.TABLE_NAME));
      }
    }
  }
  ...
  private void checkAndForceCommit(String errMsg) {
    if (!checkReady()) {
      ...
      String inflightInstant = writeClient.getInflightAndRequestedInstant(this.conf.getString(HoodieOptions.TABLE_TYPE));
  ...
  private void checkAndCommitWithRetry() {
    int retryTimes = this.conf.getInteger(HoodieOptions.RETRY_TIMES);
    long retryIntervalMillis = this.conf.getLong(HoodieOptions.RETRY_INTERVAL_MS);
    int tryTimes = 0;
  ...
  private void doCommit() {
    ...
    if (writeResults.size() == 0) {
      ...
      this.writeClient.deletePendingInstant(this.conf.getString(HoodieOptions.TABLE_TYPE), this.inFlightInstant);
      reset();
      return;
    }
    ...
    if (!hasErrors || this.conf.getBoolean(HoodieOptions.IGNORE_FAILED_BATCH)) {
